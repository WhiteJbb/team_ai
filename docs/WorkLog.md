...
## 2026-07-15 — 계획-구현 정합성 전수 검증 + 문서 표류 정정 (main 직접)

### 검증
- Plan.md의 검증 가능한 주장 42건(코어 의미론 §1~7·M1/M2 10건, M3/M4 13건,
  M5.1~M5.5 19건)을 코드·테스트와 전수 대조했다. 39건 일치, 3건 부분일치.
  전체 unittest 510개 통과(7.184s). 부분일치는 기능 결함이 아니라 이월 명시
  항목(§2 절단 보존 우선순위 세밀 규칙 — M5 compaction으로 이월, agent.py
  독스트링에 명시)과 문서 표류 2건이었다.

### 문서 표류 정정 (동작 무변 — 주석/문서만)
- `configs/team.default.yaml` — max_messages 주석이 D-033 이전 의미("메시지
  총량")로 남아 있어 실제 동작(일반 채팅만 카운트, 도달 시 proposal 전환,
  제안·투표 제외)으로 정정.
- `contracts.py` Session 독스트링·Plan.md §6 — 종료 원자성을 "세션 단위 lock +
  경합 우선순위표"로 기술했으나 실제 구현은 단일 이벤트 루프 동기 블록의
  first-wins(`_finalize` is_terminal 가드)이고 우선순위 중재는 없다. 문구를
  구현에 맞추고 우선순위표 도입 여부는 미결로 표기.
- Plan.md 디렉터리 트리 — 실존 파일(config.py, run.py, serve.py, llm/fake.py,
  llm/chatgpt_auth.py) 반영. M3 REST 열거 — `GET /sessions`(목록)·`GET /health`
  추가.

### 남은 결정(사용자)
- ~~§2 절단 보존 우선순위의 구현 시기~~ — **D-038로 해소**: 절단 관측 로그만
  추가하고 실안건 튜닝 기간 실측 빈도로 재결정 (agent.py에 경고 로그 추가).
- 종료 경합 우선순위표 도입 여부 / M5.1 실안건 튜닝 계속(표본 1/10) /
  후속 견고화 순서(요약 compaction, 실 스택 E2E, 재시작 고도화).
