...
> 여러 AI가 함께 논의하고 합의해 하나의 결론을 만드는 멀티 에이전트 의사결정 서비스

화백은 여러 LLM 에이전트가 중앙 지시자 없이 서로 직접 대화하며 사용자가 제출한
태스크를 검토하고, 제안과 투표를 거쳐 하나의 결과를 만드는 로컬 서비스입니다.
이름은 구성원의 논의와 합의로 결정을 내리던 신라의 회의체 **화백(和白)**에서
가져왔습니다.

LLM API의 tool use 위에 메시지 패싱, 합의, 예산 제어와 관측을 위한
...
- 성공한 일반 메시지 뒤에는 새 팀 입력을 기다리고, 투표에는 원문·활성 제안·직전
  교정 피드백만 전달해 자기 연속 호출과 누적 문맥 재전송을 차단
